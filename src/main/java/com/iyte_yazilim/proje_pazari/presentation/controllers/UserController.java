...
@Tag(name = "User", description = "User management endpoints. Includes profile management, password change, "
                + "profile picture upload, and account deactivation. "
                + "Most endpoints require authentication.")
public class UserController extends BaseController {

        private final IRequestHandler<GetCurrentUserProfileQuery, ApiResponse<UserProfileDTO>> getCurrentUserProfileHandler;
        private final IRequestHandler<GetUserProfileQuery, ApiResponse<UserProfileDTO>> getUserProfileHandler;
        private final IRequestHandler<UpdateUserProfileCommand, ApiResponse<UserDto>> updateUserProfileHandler;
        ...
        @GetMapping("/{userId}")
        @Operation(summary = "Get user profile by ID", description = "Retrieves any user's public profile with statistics")
        @ApiResponses(value = {
        ...
        public ResponseEntity<ApiResponse<UserProfileDTO>> getUserProfile(@PathVariable String userId) {
                ApiResponse<UserProfileDTO> response = getUserProfileHandler.handle(new GetUserProfileQuery(userId));
        ...
        @Operation(summary = "Upload profile picture", description = "Uploads a new profile picture for the authenticated user", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @io.swagger.v3.oas.annotations.media.Content(mediaType = "multipart/form-data", schema = @io.swagger.v3.oas.annotations.media.Schema(type = "object", implementation = Object.class), encoding = @io.swagger.v3.oas.annotations.media.Encoding(name = "file", contentType = "image/jpeg, image/png, image/gif, image/webp"))))
        @ApiResponses(value = {
        ...
        public ResponseEntity<ApiResponse<String>> uploadProfilePicture(
                        @io.swagger.v3.oas.annotations.Parameter(description = "Profile picture file to upload", required = true, content = @io.swagger.v3.oas.annotations.media.Content(mediaType = "multipart/form-data")) @RequestParam("file") MultipartFile file,
                        Authentication auth) {
                String userId = getCurrentUserId(auth);
        ...
        @ApiResponses(value = {
                        ...
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Validation error or incorrect current password"),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Unauthorized")
        })
