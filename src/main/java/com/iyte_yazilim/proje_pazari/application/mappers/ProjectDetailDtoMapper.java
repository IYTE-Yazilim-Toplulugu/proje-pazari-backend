...
public interface ProjectDetailDtoMapper {
    ...
    @Mapping(target = "ownerName", expression = "java(project.getOwner() != null ? project.getOwner().getFirstName() + \" \" + project.getOwner().getLastName() : null)")
    @Mapping(target = "ownerEmail", expression = "java(project.getOwner() != null ? project.getOwner().getEmail() : null)")
    @Mapping(target = "applicationCount", expression = "java(project.getApplications() != null ? project.getApplications().size() : 0)")
